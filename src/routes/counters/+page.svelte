...
<div class="space-y-6">
  <header class="flex flex-col gap-2">
    ...
    <p class="text-slate-600">Explore public counters and follow the latest activity.</p>
  </header>

  {#if data.counters.length === 0}
    <div class="rounded-lg border border-dashed border-slate-300 p-8 text-center text-slate-600">
      No public counters yet. <a href="/create" class="text-blue-600">Create one</a> to get started.
    </div>
  {:else}
      ...
      {#each data.counters as counter (counter.id)}
        ...
        >
          <div class="flex items-start justify-between">
            <div>
              <h2 class="text-lg font-semibold text-slate-900">{counter.title}</h2>
